...
\begin{abstract}
    ...
    Visualisation of the coupling matrices shows LASSO being able to break the symmetry and return the original model, while Ridge returns a symmetric coupling matrix.
    Given enough training data, Ridge regression achieves the same performance.
    ...
    Logistic regression is able to classify spin configurations as ordered/disordered with an accuracy of approximately \(\SI{70}{\percent}\), while a neural network with one hidden layer with \(\num{400}\) neurons achieves perfect accuracy within \(10\) epochs. Consequently, neural networks proved to be superior, although the choice of hyperparameters, especially the learning rate, was crucial.
\end{abstract}
\clearpage
...
Physicists study the model because it represents a magnetic lattice and can model ferromagnetic materials, and also because it has a large university class and therefore the same behaviour as many other models.
